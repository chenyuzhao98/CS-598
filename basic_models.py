...
print('The SciPy version is {0}\n'.format(
    scipy.version.full_version))  # requires SciPy >= 0.16.0
...
def extract_features(X, val_X, test_X, max_ngram_size=2):
    ...
    # np.savez("converted/X_features-"+str(max_ngram_size) +
    #          ".npz", features=X_features)
    # np.savez("converted/val_X_features-"+str(max_ngram_size) +
    #          ".npz", features=val_X_features)
    # np.savez("converted/test_X_features-"+str(max_ngram_size) +
    #          ".npz", features=test_X_features)
...
def main():
    ...
    parser.add_argument('--data', help="The input hdf5 file",
                        type=str, default='data_no_batch.h5')
    parser.add_argument(
        '--ngram', help="Maximum ngram length", type=int, default=3)
    ...
    with h5py.File(args.data, "r") as f:
        if not os.path.isfile(os.path.join("converted/X_features-"+str(args.ngram)+".npz")):
            ...
            X_features = dict(np.load(os.path.join(
                "converted", "X_features-"+str(args.ngram)+".npz")))['features'].item()
            val_X_features = dict(np.load(os.path.join(
                "converted", "val_X_features-"+str(args.ngram)+".npz")))['features'].item()
            test_X_features = dict(np.load(os.path.join(
                "converted", "test_X_features-"+str(args.ngram)+".npz")))['features'].item()
        ...
        for index, condition in enumerate(conditions):
            ...
            acc, auc = make_predictions(
                X_features, train_y, val_X_features, valid_y, test_X_features, test_y, 1)  # s)
