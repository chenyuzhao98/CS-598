...
def parse_input_csv(filename, textfield, conditions, id_field, subj_field, chart_field):
    ...
    with open(filename, 'r') as f:
        ...
        for i, row in enumerate(reader):
            if i == 0:
                field2id = {fieldname: index for index,
                            fieldname in enumerate(row)}
                print field2id
            else:
                texts.append("<padding> " * args.padding +
                             row[field2id[textfield]] + " <padding>" * args.padding)
                current_targets = []
                for c in conditions:
...
def main():
    ...
    parser.add_argument(
        '--batchsize', help="batchsize if you want to batch the data", type=int, default=1)
    ...
    if args.max_note_len > 0:
        print("SHORTENING NOTES FROM {} TO {}".format(
